...
Each filter group wraps, and below `sm` the groups stack. This is not cosmetic: a flex item defaults to `min-width: auto`, so a non-wrapping group could neither wrap nor shrink, and the Family group's five long labels set a floor far past a phone's width that pushed the **whole page** into horizontal scroll. The page otherwise needed no mobile-specific layout: the tile row and the card grid were already wrapping.
