...
- `family?`: `{ id: string; name: string; relatedPoints: string[] }` - the named cluster of other `GrammarPoint`s expressing the same core idea at a different formality/nuance. `id` is a stable slug (e.g. `"contradiction"`), `name` is a display label (e.g. `"Contradiction (But / However)"`), and `relatedPoints` holds the ids of the *other* points in the family (excludes this point's own id, symmetric - if A's family lists B, B's family lists A). All derived at build time in `gokan-dataset`; the app never hardcodes a family id/name. Surfaced by `GrammarRelatedPointsCard` on `GrammarDetailScreen` (issue #45), using `family.name` as the card header instead of a generic label.

> [!NOTE]
> `formalityLevel`/`usageNote`/`family` are populated for a growing subset of points (307/828 across 64 families as of the `gokan-dataset` branch that introduced `family`) by [gokan-dataset#4](https://github.com/gokan-dev/gokan-dataset/pull/4) and [gokan-dataset#5](https://github.com/gokan-dev/gokan-dataset/pull/5) (the latter replaced the original flat `relatedPoints: string[]` field with the named `family` object above - issue #45). All three fields are optional and inert wherever a point has none. `compiled/grammar/index/families.json` (`Record<familyId, { name, memberIds }>`, mirroring `index/jlpt.json`'s shape) is also compiled but not yet consumed anywhere in the app - reserved for a possible future family-browsing view.
