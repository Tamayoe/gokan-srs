...
- `usageNote?`: One short (~60-80 char) line covering whatever actually disambiguates this point from its near-synonyms - usually register, sometimes connotation/nuance instead (criticism, surprise, unmet expectation). Surfaced on `GrammarQuizCard` as a small unobtrusive cue near the JLPT chip (issue #42) - deliberately not shown on `GrammarIntroCard`/`GrammarDetailScreen`'s `title`/`formation`, which already answer the question this exists to disambiguate. Absent for the ~789/828 points with no note (nothing extra rendered).
- `relatedPoints?`: Ids of other `GrammarPoint`s expressing the same core idea at a different formality/nuance (symmetric - if A lists B, B should list A). Not yet surfaced anywhere in the UI (issue #42 left this as a stretch goal, not implemented).

> [!NOTE]
> `formalityLevel`/`usageNote`/`relatedPoints` are populated for 39 pilot points (the but/because/even-though/must clusters) by [gokan-dataset#4](https://github.com/gokan-dev/gokan-dataset/pull/4), which was still unmerged when this app-side support (issue #42) was implemented. All three fields are optional and inert until that PR merges and `apps/gokan-srs/dataset`'s submodule pointer is bumped past it - implemented ahead of the data landing so the two land independently, the same "dataset lags app" pattern already used for other optional fields.
...
When `point.usageNote` is set, both the normal and read-only variants render it as a small italic `text-secondary` line directly beneath the JLPT chip (issue #42 - near-synonym points like でも/しかし/けれども are otherwise ungradable from the English prompt alone, since nothing in the sentence signals which register is expected). Nothing renders for the ~789/828 points without one - no placeholder. Deliberately not shown on `GrammarIntroCard` or `GrammarDetailScreen`'s answer-adjacent areas beyond the explanation cards, since those already show `title`/`formation` and the note's whole purpose is disambiguation *without* those. Verified by hand against the dev server (the compiled dataset has no `usageNote` values yet - see the Core Data Models `GrammarPoint` note - a point was patched locally to confirm both the present and absent render paths, then reverted).
...
- `GrammarRelatedPointsCard` (`pages/grammar/GrammarRelatedPointsCard.tsx`, issue #42 follow-up): `VocabRelationshipsCard`'s equivalent for `relatedPoints` - fetches each related id via `GrammarService.loadGrammarPoint`, renders a clickable list (title + JLPT chip) navigating to `/grammar/:grammarId`, with the same cap/expand pattern (`INITIAL_COUNT = 5`, "Show all related points") since a formality cluster can run 5-6 deep. Returns `null` when `relatedPoints` is empty, same as `VocabRelationshipsCard`'s no-relationships case
