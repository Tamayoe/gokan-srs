...
- `usageNote?`: One short (~60-80 char) line covering whatever actually disambiguates this point from its near-synonyms - usually register, sometimes connotation/nuance instead (criticism, surprise, unmet expectation). Surfaced on `GrammarQuizCard` as a small unobtrusive cue near the JLPT chip (issue #42) - deliberately not shown on `GrammarIntroCard`/`GrammarDetailScreen`'s `title`/`formation`, which already answer the question this exists to disambiguate. Absent for 409/828 points (nothing extra rendered).
- `family?`: `{ id, name, relatedPoints: string[] }` - the named near-synonym family this point belongs to, if any (e.g. `family.id = "contradiction"`, `family.name = "Contradiction (But / However)"`). Replaces an earlier flat `relatedPoints?: string[]` field (issue #47) - `family.relatedPoints` is DERIVED at `gokan-dataset` build time by grouping every point that shares the same `family.id`, so it's always symmetric (if A lists B, B lists A). Surfaced by `GrammarRelatedPointsCard` (below).
...
- `GrammarRelatedPointsCard` (`pages/grammar/GrammarRelatedPointsCard.tsx`, issue #42 follow-up, updated for the `family` schema in issue #47): `VocabRelationshipsCard`'s equivalent for `family.relatedPoints` - fetches each related id via `GrammarService.loadGrammarPoint`, renders a clickable list (kana-only title, no romaji + JLPT chip) navigating to `/grammar/:grammarId`, with the same cap/expand pattern (`INITIAL_COUNT = 5`, "Show all related points") since a family can run 5-6 members deep. The heading also shows `family.name` when set (e.g. "Contradiction (But / However)"). Returns `null` when `family` is unset or its `relatedPoints` is empty, same as `VocabRelationshipsCard`'s no-relationships case
