...
│       │   ├── client/                 # search.ts - the only client-side JS on the whole site (site-wide search box, linked from every page)
...
> [!WARNING]
> **`--exclude "dictionary/*"` on gokan-srs's `--delete` sync pass is load-bearing.** `aws s3 sync --delete` removes everything in the destination absent from the source, and the dictionary lives under that prefix in the *same* bucket. Without the exclude, every app deploy silently wipes all ~39k dictionary pages and still reports success. For the same reason, only the dictionary's *first* sync pass carries `--delete` and it excludes nothing: `--delete` combined with `--exclude`/`--include` treats every filtered-out file as absent from the source and deletes it.
