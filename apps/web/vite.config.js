    ...
    plugins: [
        {
            ...
            resolveId(id) {
                ...
                let resolved = null;
                if (id === '@gokan-srs/core')
                    resolved = path.resolve(__dirname, '../../packages/core/src/index.ts');
                else if (id === '@gokan-srs/app')
                    resolved = path.resolve(__dirname, '../../packages/app/src/index.ts');
                else if (id === '@gokan-srs/ui')
                    resolved = path.resolve(__dirname, '../../packages/ui/src/index.ts');
                else if (id.startsWith('@gokan-srs/core/'))
                    resolved = path.resolve(__dirname, `../../packages/core/src/${id.slice('@gokan-srs/core/'.length)}`);
                else if (id.startsWith('@gokan-srs/app/'))
                    resolved = path.resolve(__dirname, `../../packages/app/src/${id.slice('@gokan-srs/app/'.length)}`);
                else if (id.startsWith('@gokan-srs/ui/'))
                    resolved = path.resolve(__dirname, `../../packages/ui/src/${id.slice('@gokan-srs/ui/'.length)}`);
                if (!resolved)
                    return;
