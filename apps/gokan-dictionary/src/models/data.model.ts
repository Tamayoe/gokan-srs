// Shared contract with gokan-srs's `src/models/data.model.ts` (see CLAUDE.md -> Project
// Structure -> the 5 shared model files are intentionally duplicated across both apps).
// Trimmed to just `Tags`/`TagsKey`/`TagsLookup` (part-of-speech / usage tag display) - the
// raw JMDict DTO interfaces (JMDict, Word, Kanji, Kana, Sense, ...) in the gokan-srs copy are
// build-pipeline-only types that now live in the gokan-dataset submodule, not consumed by
// either app at runtime.
